...
const star = new THREE.Shape(pts2);
const starGeometry = new THREE.ExtrudeGeometry(star, { depth: 4, bevelEnabled: true, bevelSegments: 2, steps: 2, bevelSize: 1, bevelThickness: 1 });
// Resize and rotate star (as it comes out slightly off-center);
starGeometry.applyMatrix4(new THREE.Matrix4().makeRotationZ(Math.PI / 10));
starGeometry.applyMatrix4(new THREE.Matrix4().makeScale(0.0075, 0.0075, 0.0075));
let id = 2;
const PlanetGenerator = {
    ...
    icosahedron: new THREE.IcosahedronGeometry(1, 20), // Large Icosahedron for planets
    smallicosahdron: new THREE.IcosahedronGeometry(1, 5), // Small Icosahedron for planets
    star: starGeometry,
    seed(seed) {
    ...
    pickColor() { // Color picking has a 70% of choosing a tone from a set of primary/secondary colors and a 30% of just generating a random color
        ...
        let seed = rng();
        if (seed < 0.1) {
            return new THREE.Color(0.5 + 0.5 * rng(), rng() / 3, rng() / 3);
        } else if (seed < 0.2) {
            return new THREE.Color(rng() / 3, 0.5 + 0.5 * rng(), rng() / 3);
        } else if (seed < 0.3) {
            return new THREE.Color(rng() / 3, rng() / 3, 0.5 + 0.5 * rng());
        } else if (seed < 0.4) {
            return new THREE.Color(0.5 + 0.5 * rng(), 0.5 + 0.5 * rng(), rng() / 3);
        } else if (seed < 0.5) {
            return new THREE.Color(rng() / 3, 0.5 + 0.5 * rng(), 0.5 + 0.5 * rng());
        } else if (seed < 0.6) {
            return new THREE.Color(0.5 + 0.5 * rng(), rng() / 3, 0.5 + 0.5 * rng());
        } else if (seed < 0.7) {
            return new THREE.Color(0.5 + 0.5 * rng(), 0.5 + 0.5 * rng(), rng() / 3);
        } else {
            return new THREE.Color(rng(), rng(), rng());
        }
    },
    generateRing() {
        const rng = this.rng;
        return {
            detail: 1 + 1 * (rng() * rng()),
            size: new THREE.Vector3(Math.max(1.4 + 1.2 * (rng() - 0.5), 1.4), Math.max(1.4 + 1.2 * (rng() - 0.5), 1.4), 0.5 * (0.75 + rng() * 0.5)),
    ...
    generateStar(stars) {
        ...
        if (colorSeed < 0.025) {
            starColor = new THREE.Vector3(0, 1, 1);
        } else if (colorSeed < 0.075) {
            starColor = new THREE.Vector3(1, 0.25, 0.25);
        } else if (colorSeed < 0.15) {
            starColor = new THREE.Vector3(1, 0.5, 0.25);
        } else if (colorSeed < 0.3) {
            starColor = new THREE.Vector3(1, 1, 1);
        } else {
            starColor = new THREE.Vector3(1, 1, 0);
        }
        return {
            position: starPos,
            size: 0.5 + rng(),
    ...
    generatePlanetData() {
        ...
        return {
            ...
            clouds: {
                height: 1.05 + 0.1 * rng(),
                color: new THREE.Color(0.5 + rng(), 0.5 + rng(), 0.5 + rng()),
    ...
    makePlanet(planetData, small = false) {
        ...
        // Apply vertex colors
        planetGeo.setAttribute('color', new THREE.BufferAttribute(new Float32Array(color), 3));
        const crust = new THREE.Mesh(planetGeo, new THREE.MeshStandardMaterial({ color: new THREE.Color(1.0, 1.0, 1.0), side: THREE.DoubleSide, vertexColors: true }));
        planet.add(crust);
        ...
        planetData.rings.forEach(ring => {
            ...
            const ringMesh = new THREE.Mesh(new THREE.TorusGeometry(1, 0.05, Math.round(4 * ring.detail), Math.round(10 * ring.detail)).applyMatrix4(new THREE.Matrix4().makeScale(...ring.size)).toNonIndexed(), new THREE.MeshStandardMaterial({ color: ring.color }));
        ...
        const instancedStarMesh = new THREE.InstancedMesh(starGeometry, new THREE.MeshBasicMaterial({ color: new THREE.Color(1, 1, 1) }), planetData.stars.length);
        ...
        if (!small) {
            ...
            const selectSphere = new THREE.Mesh(new THREE.SphereGeometry(1.1, 16, 16), new THREE.MeshBasicMaterial({ color: new THREE.Color(planet.selectId) }));
    ...
    spawn(x,y,z) {
        ...
        newPlanet.chunk = new THREE.Vector3(x, y, z);
