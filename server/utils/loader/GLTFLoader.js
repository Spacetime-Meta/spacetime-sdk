/**
 * This loader is modified to work in the spacetime backend.
 * all the `import { xyz } from 'three'` where changed for 
 * some THREE.xyz directly in the code to work with the 
 * following `require('three)` import
*/
var THREE = require('three');

/**
 * Also, all since loading textures throws `ReferenceError: Request is not defined`,
...
class GLTFParser {
	...
	constructor( json = {}, options = {} ) {
		...
		const isSafari = false;///^((?!chrome|android).)*safari/i.test( navigator.userAgent ) === true;
		const isFirefox = false;//navigator.userAgent.indexOf( 'Firefox' ) > - 1;
